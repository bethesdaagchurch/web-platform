        ...
        <div className="grid gap-10 md:grid-cols-[2fr_1fr_1fr]">
          <div>
            ...
            <div className="mt-4 flex gap-3">
              {socialIcons
                .filter((social) => siteSettings.socialLinks[social.key])
                .map((social) => (
                  <a
                    key={social.key}
                    href={siteSettings.socialLinks[social.key]}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={social.label}
                    className="flex h-8 w-8 items-center justify-center rounded-full border border-black/10 text-ink-muted hover:border-brand-navy hover:text-brand-navy"
                  >
